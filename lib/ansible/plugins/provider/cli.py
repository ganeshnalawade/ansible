...
class ProviderModule(ProviderBase):
    ...
    def create_connection(self, data):
        ...
        if C.USE_PERSISTENT_CONNECTIONS:
            display.vvvv('using persistent connection', pc.remote_addr)
            ...
            self._connection = connection_loader.get('network_cli', pc, sys.stdin)

        if self.socket_path is None or not os.path.exists(self.socket_path):

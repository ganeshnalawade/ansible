...
def main():
    ...
    try:
        running = connection.get(path, None, None, output=format)
        if running:
            key = path.split('/')[-1]
            running = running[key]
    ...
    try:
        if method.lower() == 'delete':
            if running:
                if commit:
                    response = connection.edit_config(path=path, method='DELETE')
                result['changed'] = True
            else:
            ...
            if diff:
                ...
                if commit:
                    response = connection.edit_config(path=path, content=diff, method=method.upper(), format=format)
                result['changed'] = True
